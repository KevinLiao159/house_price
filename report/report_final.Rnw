...
\begin{itemize}
\item 1. Split the data into train and test, 80\% and 20\% repectively.
\item 2. Train a model using train data with 5-fold cross-valdation.
\item 3. Pick the optimal hyper-parameters.
\item 4. Predict balance using the model with the optimal parameters.
\item 5. Calculate Mean Square Error.
\item 6. Record both Mean Square Error and coefficients.
\end{itemize}
